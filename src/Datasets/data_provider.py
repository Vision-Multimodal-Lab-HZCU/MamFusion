...
def average_to_fixed_length(visual_input, map_size):
    ...
    idxs = (
        torch.arange(0, num_sample_clips + 1, 1.0)
        / num_sample_clips
        * num_clips
    )
...
def collate_train(data):
    ...
    frame_videos = torch.zeros(
        len(frame_video_features), max(video_lengths), frame_vec_len
    )
...
def collate_text_val(data):
    ...
    if captions[0] is not None:
        ...
        target = torch.zeros(
            len(captions), max(lengths), captions[0].shape[-1]
        )
...
class Dataset4PRVR(data.Dataset):
    ...
    def __init__(
        self, cap_file, visual_feat, text_feat_path, cfg, video2frames=None
    ):
        self.captions = {}
        self.cap_ids = []
        self.video_ids = []
    ...
    def __getitem__(self, index):
        ...
        clip_video_feature = average_to_fixed_length(
            np.array(frame_vecs), self.map_size
        )
        ...
        frame_video_feature = uniform_feature_sampling(
            np.array(frame_vecs), self.max_ctx_len
        )
        ...
        for cap_id in cap_ids:
            ...
            cap_tensor = torch.from_numpy(l2_normalize_np_array(cap_feat))[
                : self.max_desc_len
            ]
