...
import numpy as np
from Models.model_components import clip_nce
...
class query_diverse_loss(nn.Module):
    ...
    def forward(self, x, label_dict):
        ...
        for i, label in label_dict.items():
            N_one_hot[
                label[0] : (label[-1] + 1), label[0] : (label[-1] + 1)
            ] = torch.ones((len(label), len(label)))
        ...
        N_sim_sum = torch.where(
            N_one_hot == 1, neg_exp, torch.zeros_like(neg_exp)
        ).sum(dim=0)
...
class loss(nn.Module):
    ...
    def forward(self, input_list, batch):
        ...
        clip_trip_loss = self.get_clip_triplet_loss(
            clip_scale_scores, query_labels
        )

        frame_nce_loss = self.cfg["loss_factor"][1] * self.video_nce_criterion(
            query_labels, label_dict, frame_scale_scores_
        )
        frame_trip_loss = self.get_clip_triplet_loss(
            frame_scale_scores, query_labels
        )
    ...
    def get_clip_triplet_loss(self, query_context_scores, labels):
        ...
        for i in range(v2t_scores.shape[0]):
            ...
            neg_pair_scores, _ = torch.sort(
                v2t_scores[i][np.where(labels != i)[0]], descending=True
            )
            if self.cfg["use_hard_negative"]:
                ...
                sample_neg_pair_scores = neg_pair_scores[
                    torch.randint(0, v2t_sample_max_idx, size=(1,)).to(
                        v2t_scores.device
                    )
                ]

            v2t_loss += (
                (self.cfg["margin"] + sample_neg_pair_scores - pos_pair_scores)
                .clamp(min=0)
                .sum()
            )
        ...
        _, sorted_scores_indices = torch.sort(
            mask_score, descending=True, dim=1
        )
        t2v_sample_max_idx = (
            min(1 + self.cfg["hard_pool_size"], t2v_scores.shape[1])
            if self.cfg["use_hard_negative"]
            else t2v_scores.shape[1]
        )
        sample_indices = sorted_scores_indices[
            text_indices,
            torch.randint(
                1, t2v_sample_max_idx, size=(t2v_scores.shape[0],)
            ).to(t2v_scores.device),
        ]

        t2v_neg_scores = t2v_scores[text_indices, sample_indices]

        t2v_loss = (
            self.cfg["margin"] + t2v_neg_scores - t2v_pos_scores
        ).clamp(min=0)
    ...
    def get_frame_trip_loss(self, query_context_scores):
        ...
        loss_neg_ctx = self.get_ranking_loss(
            pos_scores, pos_query_neg_context_scores
        )
        loss_neg_q = self.get_ranking_loss(
            pos_scores, neg_query_pos_context_scores
        )
    ...
    def get_neg_scores(self, scores, scores_masked):
        ...
        sample_max_idx = (
            min(sample_min_idx + self.cfg["hard_pool_size"], bsz)
            if self.cfg["use_hard_negative"]
            else bsz
        )
    ...
    def get_ranking_loss(self, pos_score, neg_score):
        ...
        return torch.clamp(
            self.cfg["margin"] + neg_score - pos_score, min=0
        ).sum() / len(pos_score)
