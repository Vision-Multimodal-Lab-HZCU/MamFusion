...
class clip_nce(nn.Module):
    ...
    def forward(
        ...
        if self.reduction:
            return torch.mean(t2v_denominator - t2v_nominator) + torch.mean(
                v2t_denominator - v2t_nominator
            )
        assert False
...
class frame_nce(nn.Module):
    ...
    def forward(self, q2ctx_scores=None, contexts=None, queries=None):
        ...
        denominator = torch.cat((x, x.permute(1, 0, 2)), dim=1).view(
            x.shape[0], -1
        )
...
class LinearLayer(nn.Module):
    ...
    def __init__(
        self, in_hsz, out_hsz, layer_norm=True, dropout=0.1, relu=True
    ):
...
class BertSelfAttention(nn.Module):
    ...
    def generate_gauss_weight(self, props_len, width):
        ...
        weight = (
            weight.view(1, -1).expand(center.size(0), -1).to(center.device)
        )
        ...
        weight = (
            w / width * torch.exp(-((weight - center) ** 2) / (2 * width**2))
        )
        return weight / weight.max(dim=-1, keepdim=True)[0]

    def forward(
        self, query_states, key_states, value_states, attention_mask=None
    ):
        ...
        attention_scores_ori = torch.matmul(
            query_layer, key_layer.transpose(-1, -2)
        )
        attention_scores_ori = attention_scores_ori / math.sqrt(
            self.attention_head_size
        )
