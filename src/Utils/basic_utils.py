...
class BigFile:
    ...
    def __init__(self, datadir):
        ...
        self.nr_of_images, self.ndims = map(
            int, open(os.path.join(datadir, "shape.txt")).readline().split()
        )
        ...
        print(
            "[%s] %dx%d instances loaded from %s"
            % (self.__class__.__name__, self.nr_of_images, self.ndims, datadir)
        )
    ...
    def read(self, requested, isname=True):
        ...
        return [x[1] for x in index_name_array], [
            res[i * self.ndims : (i + 1) * self.ndims].tolist()
...
def time_to_index(start_time, end_time, num_units, duration):
    s_times = (
        np.arange(0, num_units).astype(np.float32)
        / float(num_units)
        * duration
    )
    e_times = (
        np.arange(1, num_units + 1).astype(np.float32)
        / float(num_units)
        * duration
    )
    candidates = np.stack(
    ...
    overlaps = compute_overlap(
        candidates.tolist(), [start_time, end_time]
